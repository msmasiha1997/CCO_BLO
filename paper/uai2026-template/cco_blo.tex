...
While we present the method for a single chance constraint for clarity, many applications involve constraints $g_1,\dots,g_r$ with individual levels $\delta_1,\dots,\delta_r$:
\[
  \mathbb{P}(g_i(x,Z)\le 0)\ge 1-\delta_i,\quad i=1,\dots,r.
\]
Our bilevel construction extends by introducing one scalar auxiliary variable per constraint, yielding a small inner problem of dimension $r$ even when $d$ is large.
