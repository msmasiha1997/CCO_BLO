...
\end{equation}
The minimizer set in \eqref{eq:cvar-var} is closely related to the quantile $\Q_{1-\delta}(Y)$; in particular, if the $(1-\delta)$-quantile is unique---e.g., if $Y$ admits a continuous density that is strictly positive in a neighborhood of $q=\Q_{1-\delta}(Y)$---then the minimizer is unique and equals $q$ \citep{rockafellaruryasev2000cvar}.
...
Fix $x\in\Xc$ and define $Y:=g(x,Z)$ with $q:=\Q_{1-\delta}(Y)$.
Assume the $(1-\delta)$-quantile is unique (equivalently, the quantile set reduces to a point; for instance, this holds if $Y$ admits a continuous density that is strictly positive in a neighborhood of $q$).
Then $\arg\min_{s\in\mathbb{R}} G_\delta(x,s)$ is a singleton and the unique minimizer satisfies
\[
...
If $Y=g(x,Z)$ has no atom, then $G_\delta(x,s)$ is differentiable in $s$ with
\[
  \partial_s G_\delta(x,s)=1-\frac{1}{\delta}\mathbb{P}(Y>s).
\]
If moreover the $(1-\delta)$-quantile is unique (Lemma~\ref{lem:unique-sstar}), then the first-order condition has a unique solution and $s^\star(x)$ is unique.
...
We assume that $Y$ has a continuous distribution and admits a density $p_Y$ that is continuous at the quantile.
...
\begin{itemize}
  ...
  \item Compare against baselines: TACO; CPP-MIP/CPP-KKT/SA/SAA; plus KKTBL.
...
We compare against:
\begin{itemize}
  \item \textbf{TACO} \citep{laguelmalickackooij2021bilevel}, a bilevel/DC reformulation optimized via a bundle method.
  \item \textbf{CPP baselines} \citep{zhao2024conformal}: \textbf{CPP-MIP} and \textbf{CPP-KKT} encodings, as well as the simpler \textbf{SA} (scenario approach) and \textbf{SAA} baselines.
  \item \textbf{KKTBL}, a KKT-based bilevel baseline implemented in our codebase for the scalar inner CVaR surrogate.
\end{itemize}
All baselines use the same training samples (to construct constraints) and the same independent evaluation samples for empirical coverage reporting.

\subsection{Sweeps}
We run synthetic sweeps over (i) decision dimension $d$ (keeping training sample size fixed), (ii) training sample size $M$ (keeping $d$ fixed), and (iii) safety level $\delta$.
We additionally include a heavy-tailed univariate setting (log-normal noise) to test sensitivity to tail behavior.

\subsection{Results}
\input{tables/synthetic_results}
\todo{Replace the placeholder table by committing benchmark outputs to \texttt{results/synthetic/} and running \texttt{python scripts/make\_latex\_tables.py}.}
...
If the $(1-\delta)$-quantile is unique, this set reduces to a single point, so the minimizer is unique and equals $\Q_{1-\delta}(Y)$.
\end{proof}
...
\]
Under the assumption that $\widehat s^\star\in[q-r/2,q+r/2]$ and $\theta\le r/2$, the interval $[\xi-\theta/2,\xi+\theta/2]$ lies inside $[q-r,q+r]$, where $p_Y\ge m$.
