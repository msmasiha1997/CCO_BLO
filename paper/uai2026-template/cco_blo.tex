...
\subsection{Multiple Chance Constraints (Remark)}
We focus on a single chance constraint in \eqref{eq:cco} for clarity, but many applications impose \emph{separate} chance constraints
\begin{equation}
  \mathbb{P}\big(g_i(x,Z)\le 0\big)\ge 1-\delta_i,\qquad i=1,\dots,r,\label{eq:cco-multi}
\end{equation}
with possibly different violation budgets $\delta_i$.
Our approach extends by introducing one auxiliary scalar $s_i$ per constraint and defining an inner objective $G_{\delta_i,i}(x,s_i)$ for each $i$.
With the smoothed strongly-convex surrogate used later, each inner minimizer $s_i^\star(x)=\arg\min_{s_i}\widehat G_{\delta_i,i}(x,s_i)$ is unique, and the inner step decomposes into $r$ independent one-dimensional solves.
In the outer objective, constraint satisfaction can be encouraged by penalizing the positive parts $\sum_{i=1}^r ([s_i^\star(x)]_+)^2/(2\mu_i)$ (or using a shared $\mu$), while gradients follow from the same implicit differentiation formula applied componentwise.
